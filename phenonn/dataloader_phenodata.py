...
class PhenoDataset(torch.utils.data.Dataset):
    def __init__(
        self, data_path, pft="DB", device=None, warmupdays=365, preprocessing=identity
    ):
        if device is None:
            self.device = "cpu"
        ...
        if warmupdays > 365:
            print("can not warm up for more than a year. Warmup is set to 365" "days.")
            self.warmupdays = 365
        else:
...
def leave_onesite_out_crossvalidation(
    data_path, pft="DB", device=None, warmupdays=365, preprocessing=identity
):
