...
def run_lstm_train(
    ...
    train_loss_list = []
    valid_loss_list = []
    training_sets, validation_sets = dl.leave_onesite_out_crossvalidation(
        "../data/datasetm{}/".format(m), pft, preprocessing="normalize", device=device
    )
    for fold, (train_dataset, valid_dataset) in enumerate(
        zip(training_sets, validation_sets)
    ):
        ...
        torch.save(
            lstm.state_dict(),
            "../lstm_models/m{}_{}_{}f_".format(m, pft, nr_features) + str(fold),
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "target",
        help="target can be: gcc; rcc; gcc_lowess; rcc_lowess; gcc_norm; rcc_norm; gcc_lowess_norm; rcc_lowess_norm",
    )
    parser.add_argument(
        "batch_size",
        type=int,
