...
Usage
-----
    python -m phenonn.data_creation.filter_pixels_by_pft \\
...
def parse_args():
    ...
    p.add_argument(
        "--pft", type=int, required=True, help="ORCHIDEE PFT number 1..15 to filter on."
    )
    p.add_argument(
        "--year_start",
        type=int,
...
def main():
    ...
    out = xr.Dataset(
        {
            ...
            "longitude": (
                ["site"],
                sel["longitude"].values[keep_idx].astype(np.float32),
            ),
    ...
    print(
        f"PFT{args.pft} (axis {pft_axis}), years "
