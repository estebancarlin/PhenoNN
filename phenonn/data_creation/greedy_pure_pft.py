...
def parse_args():
    ...
    p.add_argument(
        "--out_dir", default="", help="Output dir (default: selected_pixels' dir)."
    )
    return p.parse_args()
...
def main():
    ...
    print(
        f"{n_site:,} sites, years {args.year_start}-{args.year_end}, "
    ...
    while remaining and len(claimed) < args.max_pft:
        ...
        if not placed:
            print(
                f"Stop: best remaining PFT has only {best_count} cells "
        ...
        print(
            f"  step {len(claimed)+1}: PFT{pft_num}  "
