...
def plot_loss_histories(
    ...
    ax.plot(epochs, train_loss, label="train", color="#1f77b4", marker="o",
            markersize=3)
    ax.plot(epochs, valid_loss, label="validation", color="#ff7f0e", marker="s",
            markersize=3)
...
def plot_metric_histories(
    ...
    if log_metrics is None:
        log_metrics = [m for m in metric_names
                       if "r2" not in m.lower() and "r²" not in m.lower()]
    ...
    for i, name in enumerate(metric_names):
        ...
        ax.plot(ep[:len(tr)][tr_mask], tr[tr_mask],
                label="train", color="#1f77b4", marker="o", markersize=3)
        ax.plot(ep[:len(vl)][vl_mask], vl[vl_mask],
                label="valid", color="#ff7f0e", marker="s", markersize=3)
...
def plot_pred_vs_obs(
    ...
    if hexbin and len(pred) >= 200:
        hb = ax.hexbin(obs, pred, gridsize=gridsize, cmap="viridis",
                        mincnt=1, bins="log")
        cb = fig.colorbar(hb, ax=ax, shrink=0.8)
        cb.set_label("log₁₀(count)")
    else:
        ax.scatter(obs, pred, alpha=0.5, s=12, color="#1f77b4",
                    edgecolor="none")
...
def plot_gcc_curves(
    ...
    for ax, (label, site_name, r2_val) in zip(axes, picks):
        ...
        for i, year in enumerate(years):
            ...
            ax.plot(doys, ydf[obs_col].values,
                    color=color, linewidth=1.5, alpha=0.9,
                    label=f"{year} obs" if i == 0 else f"{year} obs")
            ax.plot(doys, ydf[pred_col].values,
...
def plot_feature_distributions(
    ...
    for idx, var_name in enumerate(all_vars):
        ...
        # Compute stats
        skewness = float(((vals - vals.mean()) ** 3).mean() / (vals.std() ** 3)) if vals.std() > 0 else 0
        ...
        # Plot raw histogram
        ax.hist(vals, bins=n_bins, color=color, alpha=0.7, density=True,
        ...
        if var_name in LOG_TRANSFORM_FEATURES:
            ...
            ax.hist(vals_log, bins=n_bins, color="#ff7f0e", alpha=0.5,
                    density=True, label="log1p", edgecolor="none")
