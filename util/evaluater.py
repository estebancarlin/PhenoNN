...
import os
from util.diagnosticRTNN import (
...
class GradientAwareLoss(nn.Module):
    ...
    Input shapes
    ------------
    pred : (batch, 1, N)   — last N days predictions (typically N=2)
    target : (batch, 1, N) — last N days targets
...
def get_loss_function(loss_type, args, logger=None):
    ...
    if loss_type == "mse":
        ...
        if logger:
            logger.info(
                f"Using GradientAwareLoss (λ={grad_w}, base={base})"
            )
