    ...
    with torch.no_grad():
        for batch_number, batch in enumerate(test_loader, 1):  
            if nr_features == 6:
                ...
                X_test = torch.cat((batch['features'][:,:,:6],batch['features'][:,:,8:10]),axis=-1)
            elif nr_features == 9:
                X_test = torch.cat((batch['features'][:,:,:7],batch['features'][:,:,8:10]),axis=-1)
            else:
                X_test = torch.cat((batch['features'][:,:,:7],batch['features'][:,:,8:]),axis=-1)
    ...
    with torch.no_grad():
        for batch in data_loader:
            if nr_features == 6:
                ...
                X = torch.cat((batch['features'][:,:,:7],batch['features'][:,:,8:10]),axis=-1)
            else:
                X = torch.cat((batch['features'][:,:,:7],batch['features'][:,:,8:]),axis=-1)
...
def run_lstm_hp_tuning(     
    ...
    # k_folds = 10
    train_dataset0 = dl.PhenoDataset('../data/datasetm{}/'.format(m), pft, preprocessing = 'standardize')     
    ...
    lstm, train_losses, valid_losses, epoch_estop = train_model(train_loader, valid_loader, lstm, loss_function, optimizer, scheduler, epochs, patience = patience, nr_features = nr_features, target = target)
    ...
    if plot_learning_curve:
        ...
        plt.savefig('../plots/Fig_LearningCurve_M{}_{}_{}f_32layers_earlystop_patience30.png'.format(m, pft, nr_features))
        plt.show()      
    
    # test the lstm model 
    test_dataset = dl.PhenoDataset('../data/testdatasetm{}/'.format(m), pft, preprocessing = train_dataset0.preprocessor.standardize)
    test_loader = DataLoader(test_dataset, batch_size = 1, shuffle = False)
    test_loss = test_model(test_loader, lstm, loss_function, nr_features, target)
    
    # pred gcc
    train_pred, train_obs, train_site, train_year = model_predict(train_loader, lstm, nr_features, target, denormalize = None)  
    valid_pred, valid_obs, valid_site, valid_year = model_predict(valid_loader, lstm, nr_features, target, denormalize = None) 
    test_pred, test_obs, test_site, test_year = model_predict(test_loader, lstm, nr_features, target, denormalize = None)     
    ...
    # R2 & RMSE
    RMSE_train = np.sqrt(metrics.mean_squared_error(train_obs.reshape(-1), train_pred.reshape(-1)))
    R2_train = metrics.r2_score(train_obs.reshape(-1), train_pred.reshape(-1)) 
    RMSE_valid = np.sqrt(metrics.mean_squared_error(valid_obs.reshape(-1), valid_pred.reshape(-1)))
    R2_valid = metrics.r2_score(valid_obs.reshape(-1), valid_pred.reshape(-1)) 
    RMSE_test = np.sqrt(metrics.mean_squared_error(test_obs.reshape(-1), test_pred.reshape(-1)))
    R2_test = metrics.r2_score(test_obs.reshape(-1), test_pred.reshape(-1)) 
    
    with open('../lstm_outputs/results_lstm_hp_tuning_fixeddataset.csv', 'a', newline = '') as csvfile:
        fieldnames = ['PFT',
                      'LSTM Model', 
                      'Feature',
                      'Learning rate', 
...
if __name__ == '__main__':
    ...
    parser.add_argument('target',  help = 'target can be: gcc; rcc; gcc_lowess; rcc_lowess')
    parser.add_argument('batch_size', type = int,  help = 'the hyperparameters tuning, lr: 0.01, 0.005, 0.001, 0.0001; dropout: 0, 0.1, 0.2, 0.5; batch_size: 4, 8, 16, 32')
    args = parser.parse_args()
    
    run_lstm_hp_tuning(m = args.m, pft = args.pft, nr_features = args.nr_features, target = args.target, batch_size = args.batch_size)
