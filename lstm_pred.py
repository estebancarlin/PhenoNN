...
def run_lstm_pred (
    ...
    if pft == 'DB':
        normalizer.min = torch.Tensor([-3.5210e+01, -2.6880e+01, -1.0229e+00, 0.0000e+00, 1.7730e+01, 8.4446e+00, 0.0000e+00, 7.9700e+02])
        normalizer.max = torch.Tensor([2.6040e+01, 4.1570e+01, 3.0722e+03, 1.6573e+01, 7.3497e+02, 1.5555e+01, 2.4811e+02, 1.3410e+03])
        ...
        normalizer.min = torch.Tensor([-3.7050e+01, -2.6760e+01, 2.8549e-01, 0.0000e+00,  2.2400e+01, 7.8985e+00, 0.0000e+00, 3.1100e+02])
        normalizer.max = torch.Tensor([2.9880e+01, 4.5070e+01, 4.1225e+03, 8.2590e+00, 6.4160e+02, 1.6102e+01, 1.1260e+02, 9.7400e+02])

        model_nums = 8


    test_dataset = dl.PhenoDataset(os.path.join(input_path,'testdata/'), pft, preprocessing = normalizer, device = device)   
    ...
    for mod_num in range(model_nums):
        ...
        # load the LSTM model
        lstm.load_state_dict(torch.load(os.path.join(input_path,'lstm_models/m{}_{}_8f_{}".format(m, pft)'+ str(mod_num))))
        lstm.eval()
...
if __name__ == '__main__':   
    ...
    parser.add_argument('--m', help = 'Put the length of blocks to shuffle or "full" to not shuffle', default='full')
