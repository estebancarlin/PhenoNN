...
`phenon.data.lai_dataset` auto-detects the pixelset layout (a `site` dim).
...
Usage
-----
    python -m phenon.dataset_creation.build_pixelset_targets \\
...
from phenon.training.config import N_DEKAD_YEAR, N_PFT, PFT_FNAME, TARGETS_FNAME
...
def _load_pft_vector_array(ds: xr.Dataset) -> xr.DataArray:
    ...
    file. Inlined here (instead of importing from phenon.data.lai_dataset) so this
    preprocessing step does not pull in torch. Mirrors the loader in
    phenon.data.lai_dataset / phenon.dataset_creation.compute_valid_pixels.
    """
    rename = {}
...
def _process_year(
    year: int, sel,
    target_dir: Path, pft_dir: Path,
    out_target_dir: Path, out_pft_dir: Path,
    complevel: int,
) -> None:
    ...
    if tpath.exists():
        ...
        with xr.open_dataset(tpath, engine="netcdf4") as tgt:
            ...
            lai_full = tgt["LAI"].transpose(
                "dekad", "latitude", "longitude").values
            vals = lai_full[:, lat_idx, lon_idx].astype(np.float32)  # (36, n_site)
            dekad = tgt["dekad"].values if "dekad" in tgt.coords \
        ...
        to_netcdf_subprocess(
            out, out_target_dir / TARGETS_FNAME.format(year=year),
    ...
    if ppath.exists():
        with xr.open_dataset(ppath, engine="netcdf4", decode_times=False) as pds:
            pda = _load_pft_vector_array(pds).transpose(
                "pft", "latitude", "longitude")
            pft_full = pda.values
        ...
        enc = {"pft_frac": {"zlib": True, "complevel": complevel,
                            "_FillValue": np.float32(np.nan)}}
        out_pft_dir.mkdir(parents=True, exist_ok=True)
        to_netcdf_subprocess(
            outp, out_pft_dir / PFT_FNAME.format(year=year),
            encoding=enc, engine="netcdf4",
        )
...
def parse_args():
    ...
    p.add_argument("--selected_pixels", required=True,
                   help="selected_pixels.nc (site_id, lat_idx, lon_idx, "
                        "latitude, longitude).")
...
def main():
    ...
    for year in range(args.year_start, args.year_end + 1):
        try:
            _process_year(year, sel, target_dir, pft_dir,
                          out_target, out_pft, args.complevel)
