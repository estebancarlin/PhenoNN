...
def coarsen_2x2_nanmean(
    ...
    valid_count = (~da.isnull()).coarsen(
        {lat_name: 2, lon_name: 2}, boundary="pad",
    ).sum()
    if min_valid > 1:
...
def reindex_to_reference(
    ...
    src_lat_name: str = "lat", src_lon_name: str = "lon",
    tol: float = 0.06,
) -> xr.DataArray:
    """
    ...
    """
    return (
        da.rename({src_lat_name: "latitude", src_lon_name: "longitude"})
          .reindex(latitude=ref_lat, longitude=ref_lon,
