...
Usage
-----
    python -m phenon.dataset_creation.build_xgb_features \\
...
from phenon.training.config import DYNAMIC_FEATURES, FEATURES_FNAME
...
ROLLING_OPS: Dict[str, str] = {
    "Tmin": "mean", "Tmax": "mean", "Tmean": "mean",
    "ssrd_sum": "mean", "strd_sum": "mean", "Rn_tot": "mean",
    "VPD_max": "mean", "VPD_mean": "mean",
    "tp_sum": "sum", "PET": "sum",
}
...
def _rolling(V: np.ndarray, obs_pos: np.ndarray,
             windows: List[int], op: str) -> Dict[int, np.ndarray]:
...
def _process_year(year: int, features_dir: Path, out_dir: Path,
                  windows: List[int], sites: Optional[List[str]],
                  complevel: int) -> None:
    ...
    site_list = ([s for s in file_sites if s in set(sites)]
                 if sites else file_sites)
    if not site_list:
        print(f"  ✗ {year} skipped — no requested site in {y_path.name}")
        return

    dyn_y = _read_daily(y_path, site_list)
    dyn_ym1 = _read_daily(features_dir / FEATURES_FNAME.format(year=year - 1),
                          site_list)

    t_y = dyn_y[DYNAMIC_FEATURES[0]].shape[0]
    if dyn_ym1 is None:
    ...
    for var in DYNAMIC_FEATURES:
        ym1 = (dyn_ym1[var] if dyn_ym1 is not None
    ...
    enc = {c: {"zlib": True, "complevel": complevel,
               "_FillValue": np.float32(np.nan)} for c in cols_order}
    out_dir.mkdir(parents=True, exist_ok=True)
    to_netcdf_subprocess(out, out_dir / XGB_FNAME.format(year=year),
...
def parse_args():
    ...
    p.add_argument("--output_dir", required=True,
                   help="Where to write xgb_features_{Y}.nc.")
...
def main():
    ...
    print(f"Sites        : {'all in file' if sites is None else f'{len(sites):,} selected'}")

    for year in range(args.year_start, args.year_end + 1):
        try:
            _process_year(year, features_dir, out_dir, windows, sites,
                          args.complevel)
