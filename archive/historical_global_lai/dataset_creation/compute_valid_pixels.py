...
from phenon.training.config import (
    N_DEKAD_YEAR, N_PFT, PFT_FNAME, TARGETS_FNAME, VALID_FNAME,
)
...
def _process_year(
    year: int,
    target_dir: Path, pft_dir: Path, output_dir: Path,
    pft_threshold: float, min_valid_dekads: int,
    complevel: int,
) -> None:
    ...
    # Land pixels (sum of PFT fractions above threshold). Handle the
    # case where the PFT file is on coords slightly different from the
    # target's grid (reindex with nearest).
    pft_sum = pft.sum(dim="pft")
    if (pft_sum.sizes.get("latitude") != tgt.sizes.get("latitude")
            or pft_sum.sizes.get("longitude") != tgt.sizes.get("longitude")):
        pft_sum = pft_sum.reindex(
            latitude=tgt["latitude"], longitude=tgt["longitude"],
            method="nearest", tolerance=0.06,
        )
    land_mask = pft_sum > pft_threshold

    # LAI observation count over the 36 dekads
    valid_count = (~tgt["LAI"].isnull()).sum(dim="dekad")
    enough_obs  = valid_count >= min_valid_dekads

    valid = (land_mask & enough_obs).astype("int8").rename("valid")
    valid.attrs.update({
        "long_name":        "Usable pixel for the LAI training set",
        "description":      ("True when the pixel has Σ PFT > pft_threshold "
                             "AND at least min_valid_dekads valid LAI "
    ...
    out["valid_count"].attrs["long_name"] = ("Number of finite LAI dekads "
                                              "in the year (0..36)")
    ...
    encoding = {
        ...
        "pft_sum":     {"zlib": True, "complevel": complevel, "dtype": "float32",
    ...
    print(f"  ✓ {out_path.name}  {n_valid:,} / {n_total:,} "
...
def parse_args():
    ...
    p.add_argument("--pft_threshold",     type=float, default=0.5,
                   help="Minimum Σ PFT to call the pixel 'land' (default 0.5).")
    p.add_argument("--min_valid_dekads",  type=int,   default=26,
                   help="Minimum finite LAI obs per year (default 26 ≈ 72 %).")
...
def main():
    ...
    print(f"Criteria   : Σ PFT > {args.pft_threshold}  AND  "
    ...
    for year in range(args.year_start, args.year_end + 1):
        try:
            _process_year(
                year, target_dir, pft_dir, output_dir,
                args.pft_threshold, args.min_valid_dekads, args.complevel,
            )
