#!/usr/bin/env python3
"""
LAI Prediction — Flat CSV Training Pipeline

Reads from two flat CSV files (features.csv + targets.csv) and trains
a sequence model with Every10DaysWrapper.

Each sample is one (site_id, year) pair:
  Input  : (31 features, seq_length days)   — default seq_length=720
  Output : (1, 36) LAI observations for that year (days 5/15/25 of each month)

Usage
-----
    cd LAI/
    python -m phenocam.main_flat \\
...
from util.utils import FileUtils
from util.evaluater import get_loss_function

from phenocam.dataset_target_feature import (
    LAIDataset,
    compute_norm_stats,
    load_norm_stats,
    get_site_ids,
...
from phenocam.models.Transformerbis import BiTransformer
from util.wrappers import Every10DaysWrapper, permuteWrapper
from phenocam.dataset_target_feature import PFT_COLS
from util.diagnostics import plot_loss_histories, plot_metric_histories, make_history_dicts
...
def parse_args():
    p = argparse.ArgumentParser(description="LAI Prediction — Flat CSV")

    # Data
    p.add_argument("--features_csv", required=True,
                   help="Path to flat daily features CSV")
    ...
    p.add_argument("--experiment", default="exp_flat",
                   help="Experiment name (creates sub-folder)")

    # Split
    p.add_argument("--split_mode", default="site", choices=["site", "year"],
    ...
    p.add_argument("--val_years", default="",
                   help="Comma-separated or range validation years")
    p.add_argument("--seed", type=int, default=42)

    # Model
    p.add_argument("--type", default="lstm",
                   choices=["lstm", "gru", "transformer", "bitransformer"],
...
def main():
    ...
    logger.info(f"Feature channels: {FEATURE_CHANNELS}  |  seq_length: {args.seq_length}")
    ...
    if args.split_mode == "site":
        ...
        if not train_years or not val_years:
            raise ValueError("--train_years and --val_years are required for year split")
    ...
    if args.stats_path and os.path.exists(stats_path):
        ...
        logger.start_task("Computing normalization stats",
                          f"from {len(norm_ids)} training sites")
        norm_stats = compute_norm_stats(
            args.features_csv, args.target_csv, norm_ids,
    ...
    # ── Datasets ──
    train_ds = LAIDataset(
        args.features_csv, args.target_csv, norm_stats,
        site_ids=train_ids, seq_length=args.seq_length,
        years=train_years, normalize_target=True,
    )
    val_ds = LAIDataset(
        args.features_csv, args.target_csv, norm_stats,
        site_ids=val_ids, seq_length=args.seq_length,
        years=val_years, normalize_target=True,
    )
