...
Predicts LAI on day x using the previous 365 days of meteorological
data. Supports LSTM, GRU, Transformer, linear", "linear_perday", and FCN architectures from RTnn.

Usage examples
--------------
# Train an LSTM on all DB sites, validate on held-out sites:
...
python -m phenocam.main --data_dir ./data/DB/ --type lstm --hidden_size 128 --num_layers 2 --num_epochs 50 --batch_size 64

# Train with year-based split (train 2018-2020, validate 2021):
python -m phenocam.main \
    --data_dir ./data/DB/ \
    --type transformer \
    --split_mode year \
...
from util.utils import FileUtils, EasyDict
from util.model_utils import ModelUtils
from util.evaluater import get_loss_function
...
@torch.no_grad()
def validate(model, loader, criterion, device, dataset=None, n_target_days=1, full_year=False):
    model.eval()
    total_loss = 0.0
    all_preds = []
    all_targets = []
    ...
    r2_per_site = np.nan
    if not full_year:
        if dataset is not None and hasattr(dataset, 'samples') and hasattr(dataset, 'site_data'):
            site_preds = {}
            site_targets = {}
...
def main():
    ...
    if args.gcc_norms_csv:
        ...
        n_matched = sum(1 for f in site_files
    ...
    if args.stats_path and os.path.exists(stats_path):
        ...
        logger.start_task("Computing normalization statistics",
    ...
    if residual_csv:
        logger.start_task("Residual learning mode",
                          f"Target = obs - pred from {residual_csv}")

    # Helper to build a training dataset from a list of files
    def make_train_ds(files):
        return PhenoCamDataset(
            files, norm_stats, seq_length=args.seq_length,
            pft_list=pft_list, stride=args.stride, lai_norms=lai_norms,
            n_target_days=n_target_days, residual_csv=residual_csv,
            random_stride=args.random_stride, feature_mode=args.feature_mode,
            full_year=args.full_year,
        )

    if args.split_mode == "site":
        val_ds = PhenoCamDataset(
            val_files, norm_stats, seq_length=args.seq_length,
            pft_list=pft_list, stride=1, lai_norms=lai_norms,
            n_target_days=n_target_days, residual_csv=residual_csv,
            feature_mode=args.feature_mode, full_year=args.full_year,
        )
        ...
        def make_train_ds(files):
            return PhenoCamDataset(
                files, norm_stats, seq_length=args.seq_length,
                pft_list=pft_list, years=train_years, stride=args.stride,
                lai_norms=lai_norms, n_target_days=n_target_days,
                residual_csv=residual_csv, random_stride=args.random_stride,
                feature_mode=args.feature_mode, full_year=args.full_year,
            )

        val_ds = PhenoCamDataset(
            site_files, norm_stats, seq_length=args.seq_length,
            pft_list=pft_list, years=val_years, stride=1, lai_norms=lai_norms,
            n_target_days=n_target_days, residual_csv=residual_csv,
            feature_mode=args.feature_mode, full_year=args.full_year,
        )
    ...
    use_site_sampling = (args.sites_per_epoch > 0
                         and args.sites_per_epoch < len(all_train_files))
    rng = np.random.RandomState(args.seed)

    if use_site_sampling:
        logger.info(f"Site subsampling: {args.sites_per_epoch}/{len(all_train_files)} "
    ...
    if n_target_days > 1:
        logger.info(f"Gradient loss mode: predicting last {n_target_days} days per window")

    if args.random_stride > 0:
        logger.info(f"Random stride: {args.random_stride} samples per site-year per epoch "
                     f"(from {train_ds._all_valid_count} total valid samples)")

    if residual_csv:
        logger.info(f"Residual mode: {len(train_ds._residual_lookup)} entries loaded "
                     f"from predictions CSV")
        logger.info(f"Samples with valid residuals — train: {len(train_ds)}, val: {len(val_ds)}")
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        if use_site_sampling:
            epoch_files = list(rng.choice(
                all_train_files, size=args.sites_per_epoch, replace=False
            ))
        ...
        val_loss, val_rmse, val_r2, val_r2_persite = validate(
            model, val_loader, criterion, device, dataset=val_ds,
            n_target_days=n_target_days, full_year=args.full_year,
        )
